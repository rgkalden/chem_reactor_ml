...
base_dict_1.update((key, value*factor_base_1) for key, value in base_dict_1.items() if key not in constant_features)
base_dict_2.update((key, value*factor_base_2) for key, value in base_dict_2.items() if key not in constant_features)
lim_dict_1.update((key, value*factor_lim_1) for key, value in lim_dict_1.items() if key not in constant_features)
lim_dict_2.update((key, value*factor_lim_2) for key, value in lim_dict_2.items() if key not in constant_features)
